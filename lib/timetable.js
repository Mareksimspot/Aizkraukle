...
function screenIndex(value, index) {
  const screen = Number(value);
  if (!Number.isInteger(screen) || (screen !== 1 && screen !== 2)) {
    throw new TypeError(`Record ${index + 1}: screen must be 1 or 2`);
  }
  return screen;
}
...
export function validateTimetable(records) {
  ...
  return records.map((record, index) => {
    ...
    normalized.screen = screenIndex(record.screen, index);
    return normalized;
  });
}
